 ...
 */
export async function downloadFile(blobId: string, outputPath?: string): Promise<any> {
      ...
      if (metadataResponse.status === 200) {
        ...
        if (responseText && responseText.trim().length > 0) {
          try {
            ...
            console.log('Raw metadata response:', responseText.substring(0, 100) + '...');
          }
        } else {
          console.warn('Metadata endpoint returned empty response');
        }
      } else {
        console.warn(`Metadata endpoint returned status: ${metadataResponse.status}`);
      }
    } catch (metadataError) {
